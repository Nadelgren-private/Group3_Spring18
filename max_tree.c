...
node *leftmost_node = NULL;
node *rightmost_node = NULL;
...
/*				y_ideal-n1->se_y);*/
  	    		...
  	    		printf("%d\t plot([%lf,%lf],[%lf,%lf],'-ro');\n",n1->insert_num,x_ideal-n1->nw_x,x_ideal-n1->se_x,y_ideal-n1->nw_y,y_ideal-n1->se_y);
		}
		else 
		{
#ifdef DRAW_GRAPH
...
/*			  y_ideal-n1->nw_y);*/
	...
	//Also uncomment the following line in order to print the MATLAB syntax.
			...
			printf("%d\t plot(%lf,%lf,'ro');\n",n1->insert_num,x_ideal-n1->nw_x,y_ideal-n1->nw_y);
		}
		if(n1->left)
		{
			printf("left child:\n");
...
/*		printf("inserting 40\n");*/
		insert(leaf->type,leaf->nw_x,leaf->nw_y,leaf->se_x,leaf->se_y,leaf->slope,&temp_node->right, &temp_node);
		if(temp_node->right) 
		{
	...
	{
/*		printf("inserting 41\n");*/
		insert(leaf->type,leaf->nw_x,leaf->nw_y,leaf->se_x,leaf->se_y,leaf->slope,&leaf->left, &leaf);
		if(leaf->left) 
		{
...
/*		printf("inserting 42\n");*/
		...
		insert(leaf->type,leaf->nw_x,leaf->nw_y,leaf->se_x,leaf->se_y,leaf->slope,&temp_node->left, &temp_node);
		if(temp_node->left)
		{
	...
	{
/*		printf("inserting 43\n");*/
		insert(leaf->type,leaf->nw_x,leaf->nw_y,leaf->se_x,leaf->se_y,leaf->slope,&leaf->right, &leaf);
		if(leaf->right)
		{
			...
			if((*leaf)->type == 1)
			{
				...
				{	node *temp_node = NULL;
					...
					else
					{
						...
						delete_node(*leaf);
						//printf("inserting 1\n");
						semi_mock_insert_return_val = semi_mock_insert(type,nw_x,nw_y,se_x,se_y,slope,&(*leaf));
					}
				}
				else if(nw_x < (*leaf)->nw_x && nw_y >= (*leaf)->se_y) // The point needs inserted left
				{	//printf("inserting 2\n");
					semi_mock_insert_return_val = semi_mock_insert(type,nw_x,nw_y,se_x,se_y,slope,&(*leaf)->left);
					if((*leaf)->left)
					{
						(*leaf)->left->parent = *leaf;
				...
				{	//printf("inserting 3\n");
					semi_mock_insert_return_val = semi_mock_insert(type,nw_x,nw_y,se_x,se_y,slope,&(*leaf)->right);
					if((*leaf)->right)
					{
						(*leaf)->right->parent = *leaf;
				...
				else
				{
					...
					if(nw_x < x_proj || nw_y < y_proj) // The point is under (the extension of) the segment
					{
						...
						else // The center portion of the segment is dominated
						{
							...
							(*leaf)->se_x = nw_x;
							//printf("inserting 8b\n");
							semi_mock_insert((*leaf)->type,x_proj,nw_y,save1,save2,(*leaf)->slope,&(*leaf)->right);
							if((*leaf)->right)
							{	
								(*leaf)->right->parent = *leaf;
...
int mock_insert(int type, double nw_x, double nw_y, double se_x, double se_y, double slope, struct node **leaf) // This function mimics the actions of the insert function, takes an input point and determines whether or not that point would be added to the tree if so desired. Will be used for a fathoming rule.
{	
/*	printing = 1;*/
...
/*			printf("nw_x: %lf, se_x: %lf, upper_val: %lf\n",nw_x,se_x,upper_val);*/
			...
			{
				it++;
				rval = mock_insert(1,upper_val,(nw_y-se_y)/(nw_x-se_x)*(upper_val-nw_x)+nw_y,0.,0.,0.,&tree);
...
/*			printf("plot(%lf,%lf,'mo');\n",upper_val,se_y);*/
			rval = mock_insert(1,upper_val,se_y,0.,0.,0.,&tree);
			if(rval == -1) return rval;
			else return (*leaf)->insert_num;
		}
...
/*			printf("nw_y: %lf, se_y: %lf, upper_val: %lf\n",nw_y,se_y,upper_val);*/
			...
			{
				it++;
				rval = mock_insert(1,(nw_x-se_x)/(nw_y-se_y)*(upper_val-nw_y)+nw_x,upper_val,0.,0.,0.,&tree);
/*				printf("plot(%lf,%lf,'mo');\n",(nw_x-se_x)/(nw_y-se_y)*(upper_val-nw_y)+nw_x,upper_val);*/
			...
			upper_val = integer_objective_lb + multiplier*smallest_coef;
/*			printf("plot(%lf,%lf,'mo');\n",upper_val,se_y);*/
			rval = mock_insert(1,nw_x,upper_val,0.,0.,0.,&tree);
			if(rval == -1) return rval;
			else return (*leaf)->insert_num;
		}
	}
	...
	//printf("slope: %lf\t calculated: %lf\n",slope,(nw_y - se_y)/(nw_x-se_x));
	if(*leaf) mock_insert_return_val = (*leaf)->insert_num;
	else mock_insert_return_val = 0;
/*	node *temp_node3 = tree;*/
	if(printing && *leaf)
	{
		printf("comparing against: %d\t %.12lf,%.12lf - %.12lf,%.12lf\n",(*leaf)->type,(*leaf)->nw_x,(*leaf)->nw_y,(*leaf)->se_x,(*leaf)->se_y);
...
/*						printf("distance of %lf btwn pt and segment\n",distance);*/
					 	if(distance < epsilon*max_range)
					 	{
					 		...
					 		{
					 			yes = 1;
						 		returned_val = mock_insert(type,nw_x,nw_y,temp_nw_x,temp_nw_y,slope,&(*leaf)->left);
								if(returned_val == -1) return -1;
								mock_insert_return_val = max(mock_insert_return_val,returned_val);
							}
							if(temp_se_x < se_x)
							{
								yes = 1;
								returned_val = mock_insert(type,temp_se_x,temp_se_y,se_x,se_y,slope,&(*leaf)->right);
								if(returned_val == -1) return -1;
								mock_insert_return_val = max(mock_insert_return_val,returned_val);
							}
							if(!yes) return -1;
...
/*					printf("location 4\n");*/
					if(nw_x != (*leaf)->nw_x)
					{
					returned_val = mock_insert(type,nw_x,nw_y,(*leaf)->nw_x,y_proj,slope,&(*leaf)->left);
					if(returned_val == -1) return -1;
					mock_insert_return_val = max(mock_insert_return_val,returned_val);
					}
					if((*leaf)->nw_y != se_y)
					{
					returned_val = mock_insert(type,x_proj,(*leaf)->nw_y,se_x,se_y,slope,&(*leaf)->right);
					if(returned_val == -1) return -1;
					mock_insert_return_val = max(mock_insert_return_val,returned_val);
					}
				}
...
/*								printf("distance of %lf btwn segments\n", distance);*/
								if(distance < epsilon*max_range)
								{
									returned_val = mock_insert(type,nw_x,nw_y,x_intersect,y_intersect,slope,&(*leaf)->left);
									if(returned_val == -1) return -1;
									mock_insert_return_val = max(mock_insert_return_val,returned_val);
									if(se_x > temp_se_x)
									{
										returned_val = mock_insert(type,temp_se_x,temp_se_y,se_x,se_y,slope,
											&(*leaf)->right);
										if(returned_val == -1) return -1;
										mock_insert_return_val = max(mock_insert_return_val,returned_val);
									}
...
/*								printf("distance of %lf btwn segments\n", distance);*/
								if(distance < epsilon*max_range)
								{
									...
									if(nw_x < temp_nw_x)
									{
										returned_val = mock_insert(type,nw_x,nw_y,temp_nw_x,temp_nw_y,slope,
											&(*leaf)->right);
										if(returned_val == -1) return -1;
										mock_insert_return_val = max(mock_insert_return_val,returned_val);
									}
						...
						if( nw_x - (*leaf)->nw_x <= .00001) // first point of incoming segment is left of current segment
						{
							if((*leaf)->nw_y - nw_y_proj_c2i <= .00001 ) // || (*leaf)->se_y <= (double)se_y_proj_c2i )
							{
								   ...
								   fabs(nw_y-nw_y_proj_c2i) > .00001)
								{
									...
									//printf("inserting 27\n");
									returned_val = mock_insert(type,nw_x,nw_y,
...
/*								printf("distance of %lf btwn segments\n", distance);*/
						...
						else // first point of incoming segment lies above or to the right of current segment
						{
							   ...
							   fabs((*leaf)->se_y-se_y) > .00001)
							{
								...
								//printf("inserting 33\n");
								returned_val = mock_insert(type,(double)se_x_proj_c2i,
...
/*			printf("setting parent to incoming node\n");*/
        	...
        	(*leaf)->left = NULL;    
        	(*leaf)->right = NULL;
        	(*leaf)->insert_num = insert_level;
        	
        	if(x_ideal - se_x < leftmost_val) 
        	{
        		leftmost_val = x_ideal - se_x;
        		leftmost_val_y = y_ideal - se_y;
        		leftmost_node = *leaf;
/*        		printf("new_leftmost_val: %lf\n",leftmost_val);*/
        	}
        	
        	if(x_ideal - nw_x > rightmost_val) 
        	{
        		rightmost_val = x_ideal - nw_x;
        		rightmost_val_y = y_ideal - nw_y;
        		rightmost_node = *leaf;
        	}
...
/*						printf("(%d) deleting root\n",__LINE__);*/
						...
						insert(type,nw_x,nw_y,se_x,se_y,slope,&(*leaf),&(*leaf));
					}
				}
				else if(nw_x < (*leaf)->nw_x && nw_y >= (*leaf)->se_y) // The point needs inserted left
				{	
...
/*							}*/
						...
						else // The center portion of the segment is dominated
						{
							...
							insert((*leaf)->type,x_proj,nw_y,save1,save2,(*leaf)->slope,&(*leaf)->right,&(*leaf));
/*							if((*leaf)->right)*/
/*							{	*/
/*								(*leaf)->right->parent = *leaf;*/
/*								if((*leaf)->right->subtree_size == 1) update_depth(*leaf);*/
...
/*						printf("(%d) deleting root\n",__LINE__);*/
				...
				else 
				{
					...
					insert(type,nw_x,nw_y,(*leaf)->nw_x,y_proj,slope,&(*leaf)->left,&(*leaf));
/*					if((*leaf)->left)*/
/*					{*/
/*						(*leaf)->left->parent = *leaf;*/
/*						if((*leaf)->left->subtree_size == 1) update_depth(*leaf);*/
/*					}*/
					...
					insert(type,x_proj,save,se_x,se_y,slope,&(*leaf)->right,&(*leaf));
/*					if((*leaf)->right)*/
/*					{*/
/*						(*leaf)->right->parent = *leaf;*/
/*						if((*leaf)->right->subtree_size == 1) update_depth(*leaf);*/
						...
						&& x_intersect - se_x <= .0000001 ) // The segments intersect
					{
						...
						{	
							if( (*leaf)->nw_x < nw_x ) // Left most portion of current segment is non-dominated
							{
								...
								insert((*leaf)->type,x_intersect,y_intersect,save2,save1,(*leaf)->slope,&(*leaf)->right,&(*leaf));
/*								if((*leaf)->right)*/
/*								{*/
/*									(*leaf)->right->parent = *leaf;*/
/*									if((*leaf)->right->subtree_size == 1) update_depth(*leaf);*/
/*								}*/
								...
								insert(type,nw_x,nw_y,x_intersect,y_intersect,slope,&(*leaf)->right,&(*leaf));
/*								if((*leaf)->right)*/
/*								{*/
/*									(*leaf)->right->parent = *leaf;*/
/*									if((*leaf)->right->subtree_size == 1) update_depth(*leaf);*/
/*								}*/
								if(se_x_proj_c2i == se_x_proj_c2i)
								{
									...
									insert(type,(double)se_x_proj_c2i,save1,se_x,se_y,slope,&(*leaf)->right,&(*leaf));
/*									if((*leaf)->right)*/
/*									{*/
/*										(*leaf)->right->parent = *leaf;*/
/*										if((*leaf)->right->subtree_size == 1) update_depth(*leaf);*/
						...
						{	//printf("right side of incoming segment is below current segment\n");
							...
							{	//printf("Right most portion of current segment is non-dominated\n");
								...
								if(se_x_proj_i2c == se_x_proj_i2c)
								{
									...
									insert((*leaf)->type,(double)se_x_proj_i2c,se_y,save1,save2,
...
/*										}*/
								...
								insert(type,x_intersect,y_intersect,se_x,se_y,slope,&(*leaf)->right,&(*leaf));
/*									if((*leaf)->right)*/
/*									{*/
/*										(*leaf)->right->parent = *leaf;*/
/*										if((*leaf)->right->subtree_size == 1) update_depth(*leaf);*/
/*									}*/
								if(nw_y_proj_c2i == nw_y_proj_c2i)
								{
									...
									insert(type,nw_x,nw_y,save,(double)nw_y_proj_c2i,slope,&(*leaf)->left,&(*leaf));
/*										if((*leaf)->left)*/
/*										{*/
/*											(*leaf)->left->parent = *leaf;*/
/*											if((*leaf)->left->subtree_size == 1) update_depth(*leaf);*/
...
/*			else*/
/*			{*/
/*				insert(2,fmax(points[r],points[r+2]),fmin(points[r+1],points[r+3]),fmin(points[r],points[r+2]),fmax(points[r+1],points[r+3]),(points[r+3]-points[r+1])/(points[r+2]-points[r]),&tree);*/
/*				counter++;*/
...
node *find_first_node_right_of_val(double val, double upper_val, node *cur_node) //val is x-val to be right of, upper_val is y_val to be below
{
	node *temp_node = NULL;
...
closest_nodes *find_two_nodes_right_of_val(double val, double upper_val, node *cur_node) //val is x-val to be right of, upper_val is y_val to be below
{
...
/*			printf("changing box x lb\n");*/
		...
		if(leaf->left) reduce_box(x, y, leaf->left);
	}
	//else if(x_ideal - leaf->nw_x <= x[1] && y_ideal - leaf->nw_y >= y[0] && leaf->left) reduce_box(x, y, leaf->left);
	//else if(leaf && leaf->left) reduce_box(x, y, leaf->left);
	else if(x_ideal - leaf->nw_x <= x[1] && y_ideal - leaf->nw_y >= y[0] && leaf->right) 
	{
/*		printf("going right\n");*/
		reduce_box(x, y, leaf->right);
	}
...
/*			printf("changing box y lb\n");*/
		...
		if(leaf->right) reduce_box(x, y, leaf->right);
	}
	//else if(x_ideal - leaf->se_x >= x[0] && y_ideal - leaf->se_y <= y[1] && leaf->right) reduce_box(x, y, leaf->right);
	//else if(leaf && leaf->right) reduce_box(x, y, leaf->right);
	else if(x_ideal - leaf->se_x >= x[0] && y_ideal - leaf->se_y <= y[1] && leaf->left) 
	{
/*		printf("going left\n");*/
		reduce_box(x, y, leaf->left);
	}
}
...
void create_dual_bd()
{
	...
	if(temp)
	{
		insert(2,temp->x1,temp->y1,temp->x2,temp->y2,(temp->y1 - temp->y2)/(temp->x1 - temp->x2),&tree,&empty_node2);
		while(temp->next)
		{
			insert(2,temp->x1,temp->y1,temp->x2,temp->y2,(temp->y1 - temp->y2)/(temp->x1 - temp->x2),&tree,&empty_node2);
			temp = temp->next;
...
/*      		printf("plot([%lf,%lf],[%lf,%lf],'-go');\n",x_ideal-dual->nw_x,x_ideal-dual->se_x,y_ideal-dual->nw_y,y_ideal-dual->se_y);      		*/
      		...
      		int insert_check = 0;
      		if(dual->type == 1) insert_check = mock_insert(1,x_ideal-dual->nw_x,y_ideal-dual->nw_y,0.,0.,0.,&tree);
      		else insert_check = mock_insert(2,x_ideal-dual->nw_x,y_ideal-dual->nw_y,x_ideal-dual->se_x,y_ideal-dual->se_y,
...
double get_length(node *n1, double val)
{
    	if (n1)
    	{	
		if(mock_insert(n1->type,x_ideal-n1->nw_x,y_ideal-n1->nw_y,x_ideal-n1->se_x,y_ideal-n1->se_y,
...
void find_leftmost_with_insert_num(int num, node *n)
{
/*	printf("leftmost_val: %lf\n",leftmost_val);*/
	if(n->insert_num == num)
	{
		if(x_ideal - n->se_x < leftmost_val)
		{
			leftmost_val = x_ideal - n->se_x;
			leftmost_val_y = y_ideal - n->se_y;
			leftmost_node = n;
/*			printf("leftmost_val: %lf\n",leftmost_val);*/
		}
	}
	...
	{
/*		printf("going right\n");*/
		find_leftmost_with_insert_num(num, n->right);
	}
	if(n->left && x_ideal - n->left->se_x < leftmost_val)
	{
/*		printf("going left\n");*/
		find_leftmost_with_insert_num(num, n->left);
	}
}

void change_box_corner(box *b, double v1, double v2, node *n)
{
/*	printf("input values:\n");*/
/*	printf("plot([%lf,%lf],[%lf,%lf],'-mo');\n",v1,v1,v2,v2);*/
/*	printf("comparing to: \n");*/
/*	printf("plot([%lf,%lf],[%lf,%lf],'-ro');\n",x_ideal-n->nw_x,x_ideal-n->se_x,y_ideal-n->nw_y,y_ideal-n->se_y);*/
/*	*/
/*	printf("differences: %lf,%lf\n",v2 - (y_ideal - n->se_y), v2 - (y_ideal - n->nw_y));*/
	if(v2 - (y_ideal - n->se_y) < .000001 && v2 - (y_ideal - n->nw_y) > -.000001 )
	{
/*		printf("changing vals\n");*/
		b->nw_x = x_ideal - n->se_x;
		b->nw_y = y_ideal - n->se_y;
	}
	else if(v1 < x_ideal - n->se_x)
	{
/*		printf("going right\n");*/
/*		if(n->left) change_box_corner(b, v1, v2, n->left);*/
		if(n->right) change_box_corner(b, v1, v2, n->right);
	}
	else if(v1 > x_ideal - n->nw_x)
	{
/*		printf("going left\n");*/
		if(n->left) change_box_corner(b, v1, v2, n->left);
/*		if(n->right) change_box_corner(b, v1, v2, n->right);*/
	}
/*	else if(v2 - (y_ideal - n->se_y) < .000001 && v2 - (y_ideal - n->nw_y) > -.000001 )*/
/*	{*/
/*		printf("changing vals\n");*/
/*		b->nw_x = x_ideal - n->se_x;*/
/*		b->nw_y = y_ideal - n->se_y;*/
